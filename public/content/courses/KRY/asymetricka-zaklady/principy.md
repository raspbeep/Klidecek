...
* RSA-2048: ~10 kB/s šifrování, ~1 MB/s dešifrování (RSA dešifrování je *náročnější* než šifrování).
* X25519 (ECDH): ~10 000 operací/s = ~3 MB/s ekvivalent.

Asymetrická kryptografie se proto **nepoužívá pro šifrování objemných dat**. Praxe: [[hybridni|hybridní schéma]]:

1. **Vygeneruj náhodný symetrický klíč** $K_{\mathrm{session}}$ (typicky 256 b).
2. **Zašifruj $K_{\mathrm{session}}$ asymetricky:** $C_{\mathrm{key}} = E_{VK_B}(K_{\mathrm{session}})$.
3. **Zašifruj plaintext symetricky** s $K_{\mathrm{session}}$: $C_{\mathrm{data}} = \mathrm{AEAD}_{K_{\mathrm{session}}}(M)$.
4. **Odešli $(C_{\mathrm{key}}, C_{\mathrm{data}})$** Bobovi.

Bob:

1. **Rozšifruj** $K_{\mathrm{session}} = D_{SK_B}(C_{\mathrm{key}})$.
2. **Rozšifruj** $M = \mathrm{AEAD}^{-1}_{K_{\mathrm{session}}}(C_{\mathrm{data}})$.

Náklady na asymetrickou operaci jsou ~konstantní (jedno volání pro klíč relace), data tečou symetricky rychle. Toto je *kanonická konstrukce TLS, SSH, S/MIME, PGP, Signal*.
