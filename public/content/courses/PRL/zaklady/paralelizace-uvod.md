...
Pokud procesory běží **nezávisle** (embarassingly parallel — image rendering, Monte Carlo), žádná komunikace nepotřebná. Když potřebují **sdílet data** (matrix multiplication, graph algorithms), čas roste s množstvím komunikace.
...
| :--- | :--- | :--- |
| **Embarassingly parallel** | nezávislé podúlohy, žádná komunikace | rendering, Monte Carlo, brute-force search |
