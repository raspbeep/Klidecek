...
- $t(n) = O(\log^2 n)$
- $p(n) = O(n \log^2 n)$ (počet CE)
- $c(n) = O(n \log^4 n)$ — **není cost-optimal**
...
Každý procesor $P_i$ má dvě vstupní fronty a jednu výstupní. Slučuje dvě seřazené posloupnosti délky $2^{i-2}$ do seřazené délky $2^{i-1}$ (postupně, *pipeline* style).
...
Procesor $P_i$ začne pracovat, jakmile má na *jedné* z vstupních front posloupnost délky $2^{i-2}$ a na druhé alespoň 1 prvek. Tj. začne v cyklu $2^{i-1} + i - 1$.

### Analýza

- $P_{\log n + 1}$ začne v cyklu $2^{\log n} + \log n - 1 = n + \log n - 1$.
...
| **Odd-Even Merge Sort** | $O(\log^2 n)$ | $O(n \log^2 n)$ | $O(n \log^4 n)$ | ✗ | řadicí síť |
