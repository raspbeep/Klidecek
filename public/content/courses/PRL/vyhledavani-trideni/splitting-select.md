---
title: Parallel splitting a k-tý nejmenší prvek
---

# Parallel splitting a parallel select

Předchozí kapitola ([[vyhledavani]]) probrala *vyhledávání jednoho* prvku. Dvě související, ale obecnější úlohy jsou: **rozdělit** posloupnost vůči hodnotě (splitting) a **najít $k$-tý nejmenší** prvek (select). Obě jsou *stavebními bloky* dalších algoritmů — quicksort, median-based sort, kvantily ve statistice. Klíčově: oba algoritmy *kombinují* dříve probrané primitivy — *broadcast* (rozeslání pivota), *prefix sum* (výpočet pozic), *reduce* (sloučení). Tato kapitola je *praktická aplikace* všech komunikačních operací.

## Parallel splitting — rozdělení na L, E, G
...
| Fáze | Čas | Komentář |
| :--- | :---: | :--- |
| (i) broadcast | $O(\log N)$ | tree-distribution |
| (ii) lokální split | $O(n/N)$ | sekvenční |
| (iii) prefix sums | $O(\log N)$ | scan algoritmus |
...
**Cost-optimal!** Sekvenční byl $O(n)$, paralelní cena také $O(n)$.

### Příklad — splitting $S$ s 15 prvky vůči $m = 8$
...
## Parallel select — k-tý nejmenší prvek

**Úloha**: najít $k$-tý nejmenší prvek posloupnosti $S$ délky $n$, $1 \le k \le n$. Pokud $k = n/2$, jedná se o **medián**.

Sekvenčně: *median-of-medians* (Blum, Floyd, Pratt, Rivest, Tarjan 1973) v *deterministickém* čase $O(n)$. (Nemohlo by být rychlejší — všechny prvky se musí *aspoň* prohlédnout.)
...
**Klíčový trik**: medián mediánů $m$ leží *blízko* skutečného mediánu — garantuje, že po dělení zbývá *aspoň 30 %* vyloučených prvků. Rekurze pak konverguje *geometricky*.
...
**Klíčové pozorování**: při vhodné volbě pivota $m$ je *aspoň 30 %* prvků *vyřazeno* (jsou v $L \cup E$ nebo $E \cup G$, a tudíž ne v rekurzivně prozkoumané části). Po každé rekurzi tedy zbývá *nanejvýš 70 %* prvků.

Celková práce sumou geometrické řady:
...
(Konstanta 1/0.3 ≈ 3.33 absorbována do $O$.)
...
**Krok 4**: PARALLEL_SPLITTING s pivotem $m = 8$.
...
- **Medián**: nalezení středního prvku — $k = n/2$.
- **Kvantily**: $k$-tý percentil pro statistiku.
- **Splitter selection v sample sort**: $p - 1$ kvantilů určí $p$-prvkové splittery pro distribuci dat mezi procesy.
- **Top-K v databázích a ML**: vrátit $K$ nejlepších/nejhorších záznamů.

## Splitting jako primitivní operace

Splitting samo o sobě se používá ve více algoritmech:

- **Quicksort paralelní** — splitting s pivotem rekurzivně.
- **Distribuovaný sort** — po výběru splitterů provedeme splitting, pak local sort, pak gather/all-to-all.
...
3. **Randomizace** zjednodušuje implementaci — náhodný pivot funguje *průměrně* O(n/N), bez nutnosti median-of-medians.
4. **Knihovní implementace**: `std::nth_element` (sekvenční, intro-select); Boost.Compute pro GPU paralelní.

## Co dál

[[transposition-enumeration]] zavede konkrétní *řadicí* algoritmy: Odd-Even Transposition Sort (linear topology) a Enumeration Sort (mesh + linear bus). [[merge-radici]] probere algoritmy *slučováním* (Odd-Even Merge, Pipeline Merge, Bucket Sort). [[radici-vyber]] probere algoritmy *výběrem* — Minimum Extraction Sort a *Median Finding and Splitting* (který je *paralelní quicksort* postavený přesně na PAR_SELECT + PARALLEL_SPLITTING z této kapitoly).
