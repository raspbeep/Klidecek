    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      <div className="viz-controls" style={{ fontSize: 11 }}>
        <label style={{ display: "flex", gap: 4, alignItems: "center" }}>
          ...
          <select className="viz-select" value={regimeKey} onChange={(e) => setRegimeKey(e.target.value)}>
            {Object.entries(REGIMES).map(([k, v]) => <option key={k} value={k}>{v.label}</option>)}
          </select>
        </label>
        <label style={{ display: "flex", gap: 4, alignItems: "center" }}>
          <span style={{ color: "var(--text-muted)" }}>aktivace:</span>
          <select className="viz-select" value={nlKey} onChange={(e) => setNlKey(e.target.value)}>
