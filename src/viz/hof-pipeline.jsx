  ...
  return (
    <div style={ctn}>
      ...
      <div className="viz-controls">
        ...
        <input type="range" className="viz-slider" min="0" max="20" value={n} onChange={(e) => setN(parseInt(e.target.value))} />
        <code style={mono}>n = {n}</code>
        <span style={{ ...lbl, marginLeft: 12 }}>consumer:</span>
        {["sum", "head", "list"].map((c) => (
          <button key={c} className="viz-btn" data-active={consumer === c} onClick={() => setConsumer(c)}>{c}</button>
        ))}
      </div>
