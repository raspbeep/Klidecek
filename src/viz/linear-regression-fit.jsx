    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      <div className="viz-controls">
        <label style={{ display: "flex", gap: 6, alignItems: "center" }}>
          ...
          <input type="range" className="viz-slider" min={1} max={9} value={degree} onChange={(e) => setDegree(+e.target.value)} style={{ width: 100 }}/>
          <span style={{ fontFamily: "var(--font-mono)", minWidth: 16 }}>{degree}</span>
        </label>
        <button className="viz-btn" onClick={addPoint}>+ bod</button>
        <button className="viz-btn" onClick={() => setPoints(INITIAL_POINTS.map((p) => [...p]))}>reset</button>
