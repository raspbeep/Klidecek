...
import {
  ...
  loadLastExamSpec, saveLastExamSpec,
...
/* ─── Topbar ──────────────────────────────────────────────── */
function Topbar({ navigate, onOpenSearch, onOpenProgress, onOpenSettings, hasProgress, dark, onToggleDark, repoUrl }) {
  return (
    <header className="topbar">
      <div className="topbar-inner">
        <button className="brand" onClick={() => navigate("/")}
          style={{ appearance: "none", border: "none", background: "transparent", cursor: "default", padding: 0 }}>
          <span className="mark mark-img">
            <img src={(import.meta.env.BASE_URL || "/") + "klidecek.webp"} alt="Klideček" />
          </span>
        ...
        <div className="topbar-actions">
          ...
          <button className="icon-btn" data-active={hasProgress} onClick={onOpenProgress} title="Progress">
...
function SearchSheet({ content, defaultScopeCourseId, initialScope, navigate, onClose }) {
  ...
  const scopeCourse = scope === "course" && defaultScopeCourseId ? content.findCourse(defaultScopeCourseId) : null;
  const results = useMemo(
    () => search(index, q, { scopeCourseId: scope === "course" ? defaultScopeCourseId : null }),
  ...
  const onKeyDown = (e) => {
    ...
    else if (e.key === "Enter") { if (results[activeIdx]) { e.preventDefault(); open(results[activeIdx]); } }
    ...
    <div className="overlay search-overlay" onClick={onClose}>
      <div className="sheet search-sheet" onClick={(e) => e.stopPropagation()}>
        <div className="search-head">
          <svg className="search-head-icon" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="11" cy="11" r="7"/><path d="m20 20-3.5-3.5"/></svg>
          <input
            ref={inputRef}
            value={q}
            onChange={(e) => setQ(e.target.value)}
            onKeyDown={onKeyDown}
            placeholder={scopeCourse ? `Search ${scopeCourse.id}…` : "Search topics, subtopics, courses…"}
        ...
        {defaultScopeCourseId && (
          <div className="search-scope">
            <button data-active={scope === "course"} onClick={() => setScope("course")}>
        ...
        <div className="search-results" ref={listRef}>
          {q.trim() === "" ? (
            ...
            <button
              ...
              onClick={() => open(r)}
            >
              <span className="search-result-kind">{r.kind === "sub" ? "sub" : r.kind === "topic" ? "topic" : "course"}</span>
              <div className="search-result-text">
                ...
                {r.kind !== "course" && (
                  <div className="search-result-path">
                    ...
                    {r.kind === "sub" && r.topicTitle && <span>{r.topicTitle}</span>}
                  </div>
                )}
              </div>
              <svg className="search-result-arrow" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="m9 6 6 6-6 6"/></svg>
            </button>
          ))}
        </div>
      </div>
...
function ModeSwitcher({ route, navigate }) {
  let top = "courses";
  ...
  return (
    <nav className="modeswitch-wrap">
      <div className="modeswitch">
        {items.map((i) => (
          <button key={i.id} data-active={top === i.id} onClick={() => navigate(i.to)} title={i.label}>
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d={i.icon}/></svg>
            {i.label}
          </button>
        ))}
      </div>
    ...
    <div className="overlay" onClick={onClose}>
      <div className="sheet" onClick={(e) => e.stopPropagation()}>
        ...
        <p style={{ margin: "0 0 12px", fontSize: 13.5, color: "var(--text-muted)" }}>
          Your progress is saved in this browser's storage. Export it as JSON to back up or move to another device.
        </p>
        <div style={{ display: "flex", gap: 4, marginBottom: 14, padding: 3, background: "var(--bg-inset)", borderRadius: 8 }}>
          <button className="btn ghost" style={{ flex: 1, justifyContent: "center", background: tab === "export" ? "var(--bg-card)" : "transparent", boxShadow: tab === "export" ? "var(--shadow-sm)" : "none" }}
            onClick={() => setTab("export")}>Export</button>
          <button className="btn ghost" style={{ flex: 1, justifyContent: "center", background: tab === "import" ? "var(--bg-card)" : "transparent", boxShadow: tab === "import" ? "var(--shadow-sm)" : "none" }}
            onClick={() => setTab("import")}>Import</button>
        </div>

        {tab === "export" ? (
          <>
            <div style={{ fontSize: 13, color: "var(--text-muted)", marginBottom: 10 }}>
              <strong style={{ color: "var(--text)" }}>{count}</strong> subtopic{count === 1 ? "" : "s"} marked complete.
            </div>
            <textarea readOnly value={exportText}
              style={{ width: "100%", minHeight: 160, maxHeight: 240, padding: 10, fontFamily: "var(--font-mono)", fontSize: 11.5, background: "var(--bg-inset)", border: "0.5px solid var(--line)", borderRadius: 8, color: "var(--text)", resize: "vertical" }} />
            <div style={{ display: "flex", gap: 8, marginTop: 12, flexWrap: "wrap" }}>
              <button className="btn primary" onClick={doDownload}>Download .json</button>
              <button className="btn" onClick={doCopy}>Copy</button>
              <span style={{ flex: 1 }} />
              <button className="btn ghost" onClick={doClear} style={{ color: "var(--warn)" }}>Reset</button>
            </div>
          </>
        ) : (
          <>
            <div style={{ fontSize: 13, color: "var(--text-muted)", marginBottom: 10 }}>
              Paste a previously-exported JSON file. Existing progress is preserved — only newer marks overwrite.
            </div>
            <textarea value={text} onChange={(e) => { setText(e.target.value); setError(null); setImported(null); }}
              placeholder='{"version":1,"progress":{...}}'
              style={{ width: "100%", minHeight: 160, padding: 10, fontFamily: "var(--font-mono)", fontSize: 11.5, background: "var(--bg-inset)", border: "0.5px solid var(--line)", borderRadius: 8, color: "var(--text)", resize: "vertical" }} />
            {error && <div style={{ marginTop: 10, padding: 10, background: "oklch(0.68 0.16 65 / 0.10)", border: "0.5px solid var(--warn)", borderRadius: 8, color: "var(--warn)", fontSize: 13 }}>{error}</div>}
            {imported !== null && <div style={{ marginTop: 10, padding: 10, background: "oklch(0.62 0.15 145 / 0.10)", border: "0.5px solid var(--ok)", borderRadius: 8, color: "var(--ok)", fontSize: 13 }}>Merged {imported} entries.</div>}
            <div style={{ display: "flex", gap: 8, marginTop: 12, alignItems: "center" }}>
              ...
              <button className="btn primary" onClick={doImport} disabled={!text.trim()}>Import</button>
            </div>
          </>
        )}
      </div>
    ...
    <div className="overlay" onClick={onClose}>
      <div className="sheet" onClick={(e) => e.stopPropagation()}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
          <div>
            <div style={{ fontSize: 12, textTransform: "uppercase", letterSpacing: "0.06em", color: "var(--text-faint)", marginBottom: 8 }}>Theme</div>
            <div style={{ display: "flex", gap: 6, padding: 3, background: "var(--bg-inset)", borderRadius: 8, width: "fit-content" }}>
              {[{ v: false, label: "Light" }, { v: true, label: "Dark" }].map((o) => (
                <button key={o.label} className="btn ghost"
                  style={{ background: t.dark === o.v ? "var(--bg-card)" : "transparent", boxShadow: t.dark === o.v ? "var(--shadow-sm)" : "none", padding: "6px 14px" }}
                  onClick={() => setTweak("dark", o.v)}>{o.label}</button>
              ))}
            </div>
          </div>
          <div>
            <div style={{ fontSize: 12, textTransform: "uppercase", letterSpacing: "0.06em", color: "var(--text-faint)", marginBottom: 8 }}>Accent color</div>
            <div style={{ display: "flex", gap: 10 }}>
              {[264, 22, 142, 340, 80].map((h) => (
                <button key={h} aria-label={`accent ${h}`} onClick={() => setTweak("accentHue", h)}
                  style={{ width: 32, height: 32, borderRadius: "50%", background: `oklch(0.55 0.18 ${h})`, border: t.accentHue === h ? "2.5px solid var(--text)" : "0.5px solid var(--line)", cursor: "default" }} />
              ))}
            </div>
          </div>
          <div>
            <div style={{ fontSize: 12, textTransform: "uppercase", letterSpacing: "0.06em", color: "var(--text-faint)", marginBottom: 8 }}>Density</div>
            <div style={{ display: "flex", gap: 6, padding: 3, background: "var(--bg-inset)", borderRadius: 8, width: "fit-content" }}>
              {["compact", "regular"].map((d) => (
                <button key={d} className="btn ghost"
                  style={{ background: t.density === d ? "var(--bg-card)" : "transparent", boxShadow: t.density === d ? "var(--shadow-sm)" : "none", padding: "6px 14px", textTransform: "capitalize" }}
...
export function App() {
  ...
  useEffect(() => {
    loadContent().then(setContent).catch((e) => {
      console.error(e);
  ...
  useEffect(() => {
    ...
    if ((route.mode === "exam-spec" || route.mode === "exam-topic") && route.specId) {
  ...
  useEffect(() => {
    ...
    if (t.dark) {
      ...
      root.style.setProperty("--accent-soft", `oklch(0.72 0.18 ${t.accentHue} / 0.18)`);
      root.style.setProperty("--accent-line", `oklch(0.72 0.18 ${t.accentHue} / 0.40)`);
    } else {
      root.style.setProperty("--accent",      `oklch(0.55 0.18 ${t.accentHue})`);
      root.style.setProperty("--accent-soft", `oklch(0.55 0.18 ${t.accentHue} / 0.10)`);
      root.style.setProperty("--accent-line", `oklch(0.55 0.18 ${t.accentHue} / 0.25)`);
    }
  ...
  if (bootError) {
    return (
      <div className="loading">
        <div style={{ color: "var(--warn)", maxWidth: 400, textAlign: "center", padding: "0 20px" }}>
          <strong>Failed to load content.</strong><br />
          <span style={{ fontSize: 13, color: "var(--text-muted)" }}>{bootError}</span>
        </div>
      </div>
    );
  }
  if (!content) {
    return <div className="loading"><div className="spinner" />Loading content…</div>;
  }
  ...
  return (
    ...
    <div className="app">
      ...
      <main className="content">
        <BlockContext.Provider value={{ content, navigate }}>
          {page}
        </BlockContext.Provider>
      </main>
      <ModeSwitcher route={route} navigate={navigate} />
      {showProgress && <ProgressSheet onClose={() => setShowProgress(false)} />}
      {showSettings && <SettingsSheet t={t} setTweak={setTweak} onClose={() => setShowSettings(false)} />}
      {showSearch && (
        <SearchSheet
          content={content}
